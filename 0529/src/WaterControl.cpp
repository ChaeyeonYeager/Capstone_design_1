...
static const int     PULSE_PER_5ML = 38;  // 5 ml 당 펄스 수


void initWaterSystem() {
  pinMode(flowSensorPin, INPUT_PULLUP); // 유량 센서 핀 입력으로 설정
  pinMode(relayPin, OUTPUT);            // 릴레이 핀 출력으로 설정

  digitalWrite(relayPin, LOW);          // 시작 시 릴레이 꺼짐 상태로 설정

  Serial.println(F("💧 워터 시스템 초기화 완료"));
}
